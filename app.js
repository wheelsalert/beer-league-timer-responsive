...
async function unlockAudio(){const ctx=getAudioContext();if(ctx&&ctx.state==='suspended'){try{await ctx.resume();}catch(e){}}}
...
function playDefaultHornOnce(durationMs=DEFAULT_HORN_MS){const ctx=getAudioContext();if(!ctx)return Promise.resolve();return new Promise(resolve=>{const now=ctx.currentTime;const gain=ctx.createGain();const osc1=ctx.createOscillator();const osc2=ctx.createOscillator();osc1.type='sawtooth';osc2.type='square';osc1.frequency.setValueAtTime(410,now);osc2.frequency.setValueAtTime(205,now);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(.22,now+.02);gain.gain.exponentialRampToValueAtTime(.14,now+durationMs/1000*.6);gain.gain.exponentialRampToValueAtTime(.0001,now+durationMs/1000);osc1.connect(gain);osc2.connect(gain);gain.connect(ctx.destination);osc1.start(now);osc2.start(now);osc1.stop(now+durationMs/1000);osc2.stop(now+durationMs/1000);window.setTimeout(resolve,durationMs+50);});}
function playCustomHornOnce(){return new Promise(resolve=>{if(!customHornUrl){resolve(false);return;}try{const clip=new Audio(customHornUrl);clip.preload='auto';clip.currentTime=0;let settled=false;const finish=(played)=>{if(settled)return;settled=true;resolve(played);};clip.onended=()=>finish(true);clip.onerror=()=>finish(false);clip.play().then(()=>{window.setTimeout(()=>finish(true),Math.max(200,customHornDurationMs)+50);}).catch(()=>finish(false));}catch(e){resolve(false);}});}
async function playHornSequence(times){for(let i=0;i<times;i++){const ok=await playCustomHornOnce();if(!ok){await playDefaultHornOnce(DEFAULT_HORN_MS);}if(i<times-1){await waitMs(GAP_MS);}}}
...
async function startTimer(){if(running||completionInProgress)return;await unlockAudio();if(isFreshStart()){loadValuesFromInputs();}else{sanitizeInputs();}running=true;await playHornSequence(1);if(!completionInProgress){timerId=window.setInterval(tickTimer,1000);}}
...
function resetToDefaultsUI(){byId('secondsInput').value=String(DEFAULT_SECONDS);byId('repsInput').value=String(DEFAULT_REPS);byId('hornInput').value='';if(customHornUrl){URL.revokeObjectURL(customHornUrl);customHornUrl=null;}customHornDurationMs=1000;totalSeconds=DEFAULT_SECONDS;remainingSeconds=DEFAULT_SECONDS;totalReps=DEFAULT_REPS;currentRep=1;completionInProgress=false;setSoundStatus();updateDisplay();}
...
function loadCustomHorn(event){const file=event&&event.target&&event.target.files?event.target.files[0]:null;if(customHornUrl){URL.revokeObjectURL(customHornUrl);customHornUrl=null;}if(file){customHornUrl=URL.createObjectURL(file);customHornDurationMs=1000;try{const probe=new Audio(customHornUrl);probe.preload='metadata';probe.onloadedmetadata=function(){if(Number.isFinite(probe.duration)&&probe.duration>0){customHornDurationMs=Math.ceil(probe.duration*1000);}};}catch(e){}}else{customHornDurationMs=1000;}setSoundStatus();}
...
updateDisplay();sanitizeInputs();setSoundStatus();updateResponsiveLayout();